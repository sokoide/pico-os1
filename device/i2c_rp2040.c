...
ERR dev_i2c_open(UW unit, UINT omode) {
    Semaphore csem = {
        .initial_value = 1,
        .max_value = 1,
    };
    ...
    while (state != I2C_STS_STOP) {
        ...
        case I2C_STS_SEND:
            ...
            while (1) {
                ...
                if (sts & I2C_RAW_INTR_STAT_TX_EMPTY)
                    break;
            }
...
ERR dev_i2c_read(UW unit, UW sadr, UB* buf, SZ size, SZ* asize) {
    ERR err;

    if (unit > 1)
        return E_PAR;

    err = sk_wait_semaphore(ll_devcb[unit].semid, 1, TMO_FEVR);
    if (err < E_OK)
        return err;
    ...
    err = i2c_trans(unit);
    if (asize != NULL)
        *asize = (err == E_OK) ? size : 0;

    sk_signal_semaphore(ll_devcb[unit].semid, 1);
    return err;
}

ERR dev_i2c_write(UW unit, UW sadr, UB* buf, SZ size, SZ* asize) {
    I2cExec* p_ex;
    ERR err;
